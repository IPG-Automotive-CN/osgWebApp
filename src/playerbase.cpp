...
void ScenarioPlayer::FrameWithTp(TrajPoint tp)
{
    if (viewer_ == nullptr||viewer_->entities_.size() == 0)
    {
        return;
    }

    viewer::EntityModel* entity = viewer_->entities_[0];
    entity->SetPosition(tp.x, tp.y, tp.z);
    entity->SetRotation(tp.h, 0, 0);

    if (entity->IsMoving()&& entity->IsVehicle())
    {
