               ...
               SE_Options*             opt)
{
    ...
    camera->setProjectionMatrixAsPerspective(30.0f, static_cast<double>(width) / static_cast<double>(height), 0.5, 1E5);
    ...
    if (environment_ == nullptr || opt->GetOptionSet("enforce_generate_model"))
    {
        ...
        if (odrManager->GetNumOfRoads() > 0)
        {
            ...
            printf("No scenegraph 3D model loaded. Generating a simplistic one...\n");

            roadGeom     = std::make_unique<RoadGeom>(odrManager);
            environment_ = roadGeom->root_;

            VBOSetupVisitor vbo;
            environment_->accept(vbo);
    ...
    {
        printf("Viewer::Viewer Failed to create road mark lines!\n");
    }
    ...
    if (!(opt && opt->GetOptionSet("generate_no_road_objects")))
    {
        ...
        {
            printf("Viewer::Viewer Failed to create road signs and objects!\n");
        }
    }
    ...
    osgViewer_->setCameraManipulator(rubberbandManipulator_);
    {
    ...
    // infoTextCamera->setProjectionMatrix(osg::Matrix::ortho2D(0, 800, 0, 600));
