...
bool OpenDrive::LoadOpenDriveBuffer(void *buffer, size_t len, bool replace)
{
    printf("LoadOpenDriveBuffer\n");
    if (replace)
    {
        Clear();
    }
    ...
    pugi::xml_parse_result result = doc.load_buffer(buffer, len);
...
bool OpenDrive::LoadOpenDriveDoc(pugi::xml_node doc)
{
...
bool Position::LoadOpenDrive(void *buffer, size_t len)
{
    return (GetOpenDrive()->LoadOpenDriveBuffer(buffer, len));
}
